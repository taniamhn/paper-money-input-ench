...
Polymer({
  _template: html`
    ...
    <paper-input id="input" invalid="{{invalid}}" label="[[label]]" no-label-float="[[noLabelFloat]]" always-float-label="[[alwaysFloatLabel]]" required\$="[[required]]" value="{{_formattedValue}}" name\$="[[name]]" allowed-pattern="[0-9\\.\\,\\-]" disabled\$="[[disabled]]" readonly\$="[[readonly]]" placeholder\$="[[placeholder]]">
